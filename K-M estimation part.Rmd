...
rotterdam_1 <- Rotterdam%>%filter(hormon == 1)
rotterdam_0 <- Rotterdam%>%filter(hormon == 0)

km_fit3 <- survfit(Surv(rfstime, status) ~ rotterdam_0$chemo,data = rotterdam_0)
print(km_fit3)
km_fit4 <- survfit(Surv(rfstime, status) ~ rotterdam_1$chemo, data = rotterdam_1)
print(km_fit4)
```
